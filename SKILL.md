...
- 如果输入不是 Excel，而是知识库页面、Markdown 页面、结构化调研结论，也允许继续生成飞书交付物；此时应按“研究结论转交付物”处理，而不是卡死在“必须 Excel”上
- 若输入来自 wiki / Markdown，总结时必须保留“这是调研结论口径，不是广告后台实绩口径”的提醒
...
- 优先查找现成 Bulk 模板（如 `Campaign / Ad_Group / Keyword / Negative_Keyword / ALL`），不要自创表头
- 若无法确认官方导入列结构，至少保证：复用仓库内已验证模板、按 Campaign / Ad Group / Keyword / Negative Keyword 分文件输出、并额外输出一个 `ALL.csv` 合并版
...
- 若 `user` 身份创建飞书文档失败但 `bot` 可用，应说明文档是以 `bot` 身份创建，并提醒用户后续可补做授权或切回用户态重新创建
